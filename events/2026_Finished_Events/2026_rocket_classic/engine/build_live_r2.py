...
# live_builder_common.py (shared with build_live_r3.py). Ported 2026-08-01:
# diacritic folding before tokenization + cp1252 fallback decoding. Neither
# change alters this builder's output — R2's sources are pure ASCII/UTF-8 —
# verified by diffing build() before/after against the frozen r2_live.json.
...
#
# Hourly extraction (HOURLY_RE_STRICT + extract_hourly) now lives in
# live_builder_common.py, shared with build_live_r3.py's strict tier — R2's
# source is clean, so it never needs the narrative fallback tier R3 added.
# TEE_WINDOW_RE and the result-dict shape stay local: R2's frozen output has
# no "parse_method" key, and summarize_hourly() would add one.
...
def parse_weather() -> dict:
    ...
    hourly, parse_rate = extract_hourly(HOURLY_RE_STRICT, candidate_lines)
