...
// ── "Last, First" → "First Last" conversion for tee-times artifact names ─────
...
const SCENARIO_TRAITS = [
  ...
  { key: 'ott_positional',label: 'OTT Overall',          defaultWeight: 0.12 },
  { key: 'app_overall',   label: 'App Overall',           defaultWeight: 0.20 },
  { key: 'sg_putt',       label: 'SG: Putting',          defaultWeight: 0.10 },
  { key: 'recent_form',   label: 'Recent Form (L20)',    defaultWeight: 0.10 },
...
function resolveCumKey(label) {
  ...
  const rules = [
    ...
    [/positional|ottpos|ottoverall/,      'ott_positional'],
    [/appoverall|appgen|overallapproach/, 'app_overall'],
    [/sgputt|putting/,                   'sg_putt'],
    [/recentform|l20form/,               'recent_form'],
...
async function switchRound(r) {
  ...
  try {
    ...
    liveSection?.classList.remove('loading-blur');
    if (pending) {
      pending.innerHTML = renderEmptyState('pending', `Round ${r} analysis not yet available — board shows pre-tournament model.`);
      pending.classList.remove('hidden');
    }
    content?.classList.add('hidden');
  }
      ...
      <table class="live-table">
        ...
        <tbody>${(() => {
          ...
          return sep + `<tr data-player="${esc(canonName(r.r1_name))}" data-player-id="${esc(String(_rpid))}"${isElim ? ' style="opacity:.6"' : ''}>
            ...
            <td class="sans" style="color:${r.sg_tot == null ? 'var(--text-3)' : (r.sg_tot>=0?'var(--text-1)':'var(--accent)')}">${sgFmt(r.sg_tot)}</td>
            <td class="sans" style="color:${r.sg_app == null ? 'var(--text-3)' : (r.sg_app>=0?'var(--green-ok)':'var(--accent)')}">${sgFmt(r.sg_app)}</td>
            <td class="sans" style="color:${r.sg_putt == null ? 'var(--text-3)' : (r.sg_putt>=0?'var(--navy)':'var(--accent)')}">${sgFmt(r.sg_putt)}</td>
