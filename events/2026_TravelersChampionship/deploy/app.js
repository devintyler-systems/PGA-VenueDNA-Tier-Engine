...
async function init() {
  ...
  /* ── Round 1 analysis (optional — generated by build_r1_analysis.py) ── */
  try {
    r1Data = await fetch(DATA_DIR + 'r1_analysis.json').then(r => r.json());
    console.info('[VenueDNA] Round 1 analysis loaded:', r1Data.match_summary);
  } catch (_) { /* optional */ }
  ...
  const modelVsRealHTML = `
    <div class="ri-card">
      ...
      <div class="ri-note" style="margin-bottom:.5rem">
        Trait delta = pre-tournament trait profile avg (top-10 R1 leaders vs full field).
      ...
      <div class="ri-reality-grid">
        <div class="ri-reality-item ${(appDelta ?? 0) > 6 ? 'ri-reality-yes' : 'ri-reality-mixed'}">
          ...
          <div class="ri-reality-a">${(appDelta ?? 0) > 6
            ? `Yes &mdash; APP trait delta +${appDelta?.toFixed(0)} favored R1 leaders; SG:APP +${(sg.top10.sg_app??0).toFixed(2)} vs field.${ciLoaded && ta.app_wedge?.enrichment?.available ? ` Fairway Prox confirmed: leaders hit approach ${(ta.app_wedge.enrichment.delta_primary/12).toFixed(1)}ft closer to pin.` : ''}`
            : 'Mixed evidence.'}</div>
        </div>
...
function showRoundPanel(round, ms, body) {
  ...
  if (round === 'pre') {
    ...
    const roundLabels = { r1: 'Round 1', r2: 'Round 2', r3: 'Round 3', final: 'Final' };
