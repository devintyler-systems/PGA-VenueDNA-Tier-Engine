...
const ROUND_PAYLOADS = {
  pre:   'data/FETCH_URL_PLACEHOLDER',
  r1:    'data/EVENT_SLUG_PLACEHOLDER_rd1_payload.json',
  r2:    'data/EVENT_SLUG_PLACEHOLDER_rd2_payload.json',
  r3:    'data/EVENT_SLUG_PLACEHOLDER_rd3_payload.json',
  final: 'data/EVENT_SLUG_PLACEHOLDER_final_payload.json',
};
...
async function init() {
  ...
  try {
    const [boardJson, analysisJson, briefsJson] = await Promise.all([
      fetch('data/FETCH_URL_PLACEHOLDER').then(r => r.json()),
      fetch('data/EVENT_SLUG_PLACEHOLDER_final_analysis.json').then(r => r.json()).catch(() => ({})),
      fetch('data/EVENT_SLUG_PLACEHOLDER_player_briefs.json').then(r => r.json()).catch(() => ({})),
    ]);
    ...
    fetch('data/EVENT_SLUG_PLACEHOLDER_weather_forecast.json')
...
function updateBoardSub(n) {
  ...
  if (el) el.textContent = n === total
    ? `${total} players · VENUE_NAME_PLACEHOLDER VTS`
    : `${n} of ${total} · filtered`;
}
    ...
    <div class="modal-body">
      ...
      <div class="modal-sec">
        ...
        <div class="modal-layers">
          ...
          <div class="layer-row"><div class="layer-lbl sans">Δ Fit</div><div style="flex:1;font-size:.78rem;color:var(--muted)">VENUE_NAME_PLACEHOLDER uplift</div><div class="layer-val sans" style="color:${(p.delta_fit||0)>=0?'var(--green-ok)':'var(--accent)'}">${sgSign(p.delta_fit)}</div></div>
        </div>
...
const FM = {
  ...
  'VenueDNA_flag_course_debut':       { lbl:'DEB',   cls:'flag-info',   full:'VENUE_NAME_PLACEHOLDER debut — no course history adjustment available' },
  'VenueDNA_flag_limited_history':    { lbl:'LTD',   cls:'flag-info',   full:'Limited course history — fewer than 4 starts at VENUE_NAME_PLACEHOLDER' },
...
const uiGlossary = {
  'Score Components': [
    ...
    { term: 'SG Sim', def: 'Similar-course strokes gained — sample-weighted regression using the top-20 courses most similar to VENUE_NAME_PLACEHOLDER.' },
    { term: 'Δ Fit', def: 'Delta between SG Sim and SG Base — VENUE_NAME_PLACEHOLDER venue fit signal. Positive = course-specialist upside; clamped ±0.50 SG/round.' },
  ...
  'Traits': [
    ...
    { term: 'VenueFitDelta', def: 'How much VENUE_NAME_PLACEHOLDER specifically helps or hurts this player vs. their all-course baseline.' },
    { term: 'VenueHistoryDelta', def: 'Course-history adjustment — derived from prior starts at VENUE_NAME_PLACEHOLDER. Clamped to a modest modifier.' },
    { term: 'App 150–200 yd', def: 'Approach strokes gained from 150–200 yards. The #1 birdie creation mechanism at VENUE_NAME_PLACEHOLDER (46% of approaches from this range).' },
  ...
  'Anti-Pattern Flags': [
    ...
    { term: 'DEB — Course Debut', def: 'No prior VENUE_NAME_PLACEHOLDER starts — no course history adjustment available.' },
  ],
  'Analyst Mode': [
...
async function switchRound(r) {
  ...
  if (r === 'pm') {
    ...
    try {
      const resp = await fetch('data/EVENT_SLUG_PLACEHOLDER_post_mortem_analysis.json');
  ...
  try {
    const [resp, cumLearn] = await Promise.all([
      fetch(`data/EVENT_SLUG_PLACEHOLDER_r${r}_analysis.json`),
      fetch('data/EVENT_SLUG_PLACEHOLDER_cumulative_learning.json').then(res => res.ok ? res.json() : null).catch(() => null),
    ]);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
