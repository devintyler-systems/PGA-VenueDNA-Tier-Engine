...
## Amendment 3 — 2026-08-06 (Phase 4.2 narrow producer path-binding integration)

This amendment records the first production-code integration of the schema defined above. It does not reopen or reverse any prior decision.

**What was integrated.** `engine/enrich_cards.py`'s `main()` now, after `load_pre_event_context()` and `require_supported_context()` succeed, reads and JSON-decodes `context.event_root / "input" / "source_manifest.json"`, resolves it through the already-committed, unmodified `engine/source_manifest_resolver.py::resolve_source_manifest()`, and binds every one of the thirteen required logical source roles (§9.5) to its manifest-declared physical path for every subsequent CSV load. A missing manifest, unreadable manifest, invalid JSON, non-object JSON root, or any resolver blocker (including a `sha256`/`row_count` integrity mismatch) fails release before any source file is opened, before identity resolution, before scoring, and before output/deploy directory creation or any write. `tests/test_enrich_cards.py` gained a matching synthetic-`tmp_path` test suite; `engine/source_manifest_resolver.py` itself was not modified.

**Capability-gate containment remains in force.** `require_supported_context()` — unchanged, still checked immediately after `EventContext` validation and strictly before source-manifest resolution — continues to fail a mismatched `event_slug`/`venue_slug` pairing before the producer ever attempts to locate or parse `source_manifest.json`. This integration replaces only *how a declared logical role's physical file is located*; it does not generalize `engine/enrich_cards.py` to any event or venue beyond `2026_3m_open` / `tpc_twin_cities`.

**No fallback.** Once a manifest exists, `ALL_COURSES_FILES`, `SIM_COURSES_FILES`, and every other previously-hardcoded physical filename are no longer used for production path construction; they remain defined in `engine/enrich_cards.py` only as historical/illustrative reference (matching this document's own §9.5 illustrative table), consistent with `standards/02_PGA_VENUEDNA_SCORING_SPEC.md` §2A.

**Confirmed unchanged by this amendment:**
- No event was initialized and no real `source_manifest.json` instance was created under any `events/` directory — synthetic manifests exist only inside `tests/test_enrich_cards.py`'s `tmp_path` fixtures.
- No venue-generalization migration occurred — the capability gate, the TPC-specific narrative copy in `build_headline()`/`build_win_case()`, and the venue trait weights are untouched.
- No scoring, identity, payload shape, `schemaVersion`, deploy filename, formula metadata, rank, tier, probability, penalty, gate, database, or archive behavior changed. `engine/venuedna_scoring.py`, `engine/identity_resolver.py`, `engine/event_context.py`, and `engine/build_event_package.py` were not modified.
- `config/active_event.json` was not modified and no `source_manifest_file` config pointer was added.

**Explicitly still deferred (unchanged by this amendment):** whether `config/active_event.json` ever gains an additive `source_manifest_file` pointer field; reconciling this schema's name with either `standards/04` §3D's output-side `source_manifest` payload key or `engine/build_event_package.py`'s separate presence-map object; authoring an actual `source_manifest.json` for any real event; and wiring `tpc_twin_cities_CH.csv` course-history rows into `VenueHistoryEvidence` (the `venue_history` role is now path-bound through the manifest, but `main()` still calls `compute_player_projection(..., venue_history_evidence=None)` unconditionally — this pre-existing gap is unchanged, separate technical debt, out of scope for this amendment).
